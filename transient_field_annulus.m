function FIELD = transient_field_annulus(step_duration,dish_power_per_sqr_m,FIELD,wind_velocity,T_ambient,humidity)
...
%   Detailed explanation goes here
        ...
        N = size(FIELD_0.ANNULUS.SECTION,2); % amount of sections to be calculated
        ...
        if  mdot > 0.2%dish_power_per_sqr_m>350
            ...
            T_in_receiver = FIELD_0.ANNULUS.SECTION{1}.cold_temperature1;
                ...
                while abs(dT_out_HEX)>0.01 || abs(dmdot)>0.0001
                    ...
                    FIELD.ANNULUS  = annulus(step_duration,FIELD_0.ANNULUS,mdot,wind_velocity,T_ambient);
%                         FIELD.ANNULUS.SECTION{N}.cold_temperature2(FIELD.ANNULUS.SECTION{N}.cold_temperature2<FIELD_0.HEX.WS.temperature+FIELD_0.HEX.HP.pinch) = FIELD_0.HEX.WS.temperature+FIELD_0.HEX.HP.pinch;
                    ...
                    % fit to HEX outlet, by altering T inlet receiver:
                        ...
                        dT_out_HEX(dT_out_HEX<0.5) = dT_out_HEX.*mdot./mdot1;
...
%                             FIELD.ANNULUS  = annulus(step_duration,FIELD_0.ANNULUS,0,wind_velocity,T_ambient);
...
%                     
%                              F_dT_out_HEX = @ (T_in_receiver) transient_dT_out_HEX(T_in_receiver,step_duration,dish_power_per_sqr_m,FIELD_0,wind_velocity,T_ambient,humidity);
...
%                         FIELD.ANNULUS.SECTION{N}.cold_temperature2(FIELD.ANNULUS.SECTION{N}.cold_temperature2<FIELD_0.HEX.WS.temperature+FIELD_0.HEX.HP.pinch) = FIELD_0.HEX.WS.temperature+FIELD_0.HEX.HP.pinch;
                    ...
                    % fit to HEX inlet, by altering T outlet receiver:
                        T_in_HEX = FIELD.ANNULUS.SECTION{N}.hot_temperature2
                        dT_in_HEX = (T_in_HEX_design - T_in_HEX).*(T_out_receiver./T_in_HEX);
                        ...
                        dmdot = mdot - mdot1;
                        m=m+1
                        if (m>50 && T_in_receiver>T_ambient) || m>100; break; end
...
function dT_out_HEX = transient_dT_out_HEX(T_in_receiver,step_duration,dish_power_per_sqr_m,FIELD,wind_velocity,T_ambient,humidity)
%UNTITLED Summary of this function goes here
%   Detailed explanation goes here
...
function ANNULUS  = annulus(step_duration,ANNULUS,mdot,wind_velocity,T_ambient)
%UNTITLED Summary of this function goes here
%   Detailed explanation goes here        
    ...
    for s = 1:steps
            ...
            for i = 1:N
                    ...
                    if mdot > 0.2
                            ...
                            ANNULUS.SECTION{i}.cold_temperature2(ANNULUS.SECTION{i}.cold_temperature2<T_ambient) = T_ambient;
                            ANNULUS.SECTION{i}.cold_temperature1(ANNULUS.SECTION{i}.cold_temperature1<T_ambient) = T_ambient;
                            ANNULUS.SECTION{i}.hot_temperature2(ANNULUS.SECTION{i}.hot_temperature2<T_ambient) = T_ambient;
                            ANNULUS.SECTION{i}.hot_temperature1(ANNULUS.SECTION{i}.hot_temperature1<T_ambient) = T_ambient;
...
%   Detailed explanation goes here
        ...
        index1 = min(find(SECTION.mesh(4,:)==-1))+2;
        ...
        mesh_map = SECTION.mesh;
       
    if mdot > 0.2
        
        mesh_map(1,1) = Thot1;
        mesh_map(1,index1) = Tcold1;
        
                T_mesh1 = real(Tt(1,mesh_map,mdot,Phot1,Pcold1,wind_velocity,T_ambient)); 
                
                Thot_dm = T_mesh1(1,1);
                Tcold_dm = T_mesh1(1,index1);
...
% HOT dT____________________________________________________________________________________________________________________                                
                                ...
                                Thot2_dL = Tcold1+real(exp(Ahot + Bhot.*L));
                                
                            if isnan(Thot2_dL)==1 || Thot2_dL>Thot1
                                dThotm = Thot_dm - Thot1 ;%abs(Thot_d2m - Thot2); dT2hotm(dT2hotm>10) = 10;
                                Thot2_dL = Thot1 - L.*dThotm;
                            end
                            
                                Thot2_dL(isnan(Thot2_dL) || Thot2_dL<Tcold_dm || Thot2_dL<T_ambient) = mesh_map(1,3);%Tcold_dm;                        
%___________________________________________________________________________________________________________________________                
% COLD dT__________________________________________________________________________________________________________________
                                ...
                                dT2coldm = (Tcold_dm - Tcold1); 
                                
                                if dT2coldm<0
                                    
                                    Acold = log(Tcold1-T_ambient);
                                    Bcold = log(Tcold_dm-T_ambient) - Acold;
                                    Tcold2_dL = T_ambient+real(exp(Acold + Bcold.*L));

                                else

                                    Tcold2_dL = abs(Tcold1 + L.*dT2coldm);                                    
                                
                                end
                                
                                Tcold2_dL(isnan(Tcold2_dL) || isinf(Tcold2_dL || Tcold2_dL<mesh_map(1,index1+2)) || Tcold2_dL<T_ambient) = mesh_map(1,index1+2); 
...
%___________________________________________________________________________________________________________________________ 
                ...
                mesh_map(6,1) = Tcold2_dL;
                mesh_map(6,index1) = Tcold2_dL;

                T_mesh2 = real(Tt(2,mesh_map,mdot,Phot2,Pcold2,wind_velocity,T_ambient));  

                SECTION.hot_pressure2 = Phot2;
                SECTION.cold_pressure2 = Pcold2;
                T_mesh2(6,1) = Thot2_dL;
                T_mesh2(6,index1) = Tcold2_dL;
                SECTION.hot_temperature2 = T_mesh2(6,1);
                SECTION.cold_temperature2 = T_mesh2(6,index1);  
                SECTION.mesh(1,:) = T_mesh1(1,:);
                SECTION.mesh(6,:) = T_mesh2(6,:);
       
    else
        
                Phot = P_system;
                Pcold = P_system;
                T_mesh1 = Tt(1,mesh_map,0,Phot,Pcold,wind_velocity,T_ambient);
                T_mesh2 = Tt(2,mesh_map,0,Phot,Pcold,wind_velocity,T_ambient);
                T_mesh2(isnan(T_mesh2)==1) = T_mesh1(isnan(T_mesh2)==1);
                ...
                SECTION.cold_temperature1 = T_mesh1(1,index1);
                SECTION.hot_temperature2 = T_mesh2(6,1);
                SECTION.cold_temperature2 = T_mesh2(6,index1);  
                SECTION.mesh(1,:) = T_mesh1(1,:);
                SECTION.mesh(6,:) = T_mesh2(6,:); 
                ...
                SECTION.mesh(1,end-1) = 0;
                SECTION.mesh(1,end) = 0;
                SECTION.mesh(6,end-1) = 0;
                SECTION.mesh(6,end) = 0;
end

function mesh_map = Tt(side,mesh_map,mdot,Phot,Pcold,wind_velocity,T_ambient)
%UNTITLED Summary of this function goes here
%   Detailed explanation goes here
...
%                 end
    ...
    Ts = dTdt(Ts,Rs,drs,material_tag,material_quality,mdot,Phot,Pcold,wind_velocity,T_ambient);
...
function Ts = dTdt(Ts,Rs,drs,material_tag,material_quality,mdot,Phot,Pcold,wind_velocity,T_ambient)
%UNTITLED Summary of this function goes here
%   Detailed explanation goes here
global  DATA...
    INPUT...
...
% HTS Parameters:
    ...
    index1 = min(find(material_tag==-1));
    index2 = max(find(material_tag==-1));
    ...
    T_dt_old = Ts+K0;
    T_inner_air = Ts(1) +K0;
    T_outer_air = Ts(index1+2)+K0; %aluminium foil/liner - ia always at constant thickness
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                                ...
                                % Inner Duct Boundary:
                                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                                    FoC = FoX(Ts(3),material_tag(4),material_quality(4),drs(4),dt,FoC0); %FoX(T,tag,quality,dr,dt,FoC0)
                                    
                                    if mdot <=0.2
                                        air_dens = Phot.*100000./278.05./T_inner_air;
                                        volume = pi.*Rs(3).^2;
                                        mass = air_dens.*volume;
                                        h = natural_convection_coefficient(Ts(3),Ts(1),Rs(3),0,Pcold);
                                    else
                                        h = convection_coefficient(Ts(1),Rs(3),0,Rs(3),mdot,Phot); 
                                    end

                                    k = insulation_k(material_tag(4),material_quality(4),(Ts(3)+Ts(4))./2);
%                                     if material_tag(4)==2;k = k_1st_inner_insulation((Ts(3)+Ts(4))./2); end
%                                     if material_tag(4)==3; k = k_2nd_inner_insulation((Ts(3)+Ts(4))./2); end
%                                     if material_tag(4)==4; k = k_aluminium((Ts(3)+Ts(4))./2); end 
                                    ...
                                    am = Rs(3)-drs(4)./2;
                                    A = am./a;
                                    if am<=0; a = Rs(3)+drs(4)./2; am = R(3); A = am./a; end
                                    T_dt_new(3) = T_dt_old(3).*(1-2.*FoC.*(1+Bi.*A))+2.*FoC.*(T_dt_old(4)+Bi.*A.*T_inner_air);%T_dt_old(3) + 2.*FoC.*(T_dt_old(4) - T_dt_old(3) -Bi.*A.*(T_dt_old(3) - T_inner_air));
                                    dQ_inner_air = 2.*pi.*Rs(3).*h.*(T_dt_new(3)-T_inner_air);  
                                    
                                    if mdot<=0.2
                                        dT_inner_air = dQ_inner_air./(mass.*air_Cp(T_inner_air-K0).*1000);
                                    else
                                        dT_inner_air = dQ_inner_air./(mdot.*air_Cp(T_inner_air-K0).*1000);
                                    end
                                    
                                    T_inner_air_new = T_inner_air + dT_inner_air;
%                                     T_inner_air_new(T_inner_air_new>T_inner_air) = T_inner_air + dT_inner_air;% - abs(dTgas1_by_components);
%                                     T_inner_air_new(T_inner_air_new>Tsteady.hot+K0) = Tsteady.hot+K0;
                                    T_dt_new(1) = T_inner_air_new;
                                    T_dt_old(3) = T_dt_new(3);


                                    % Outer Duct Inner surface Boudary:    
                                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                                    FoC = FoX(Ts(index1),material_tag(index1-1),material_quality(index1-1),drs(index1-1),dt,FoC0);
                                    
                                    if mdot <=0
                                        h = natural_convection_coefficient(Ts(index1),Ts(index1+2),Rs(index1),0,Pcold);
                                    else
                                        h = convection_coefficient(Ts(index1+2),Rs(index1),Rs(index1),Rs(index1+4),mdot,Pcold);
                                    end
                                    h2r_ic = h;
                                   
                                    k = insulation_k(material_tag(index1-1),material_quality(index1-1),(Ts(index1)+Ts(index1-1))./2);
%                                     if material_tag(index1-1)==2; k = k_1st_inner_insulation((Ts(index1)+Ts(index1-1))./2); end
%                                     if material_tag(index1)==3; k = k_2nd_inner_insulation((Ts(index1)+Ts(index1-1))./2); end
%                                     if material_tag(index1)==1; k = k_steel((Ts(index1)+Ts(index1-1))./2); end                               
                                    Rk = log((Rs(index1)+drs(index1))./Rs(index1))./(2.*pi.*k_aluminium(Ts(index1)));
                                    Rh = 1./(2.*Rs(index1).*pi.*h);
                                    U = 1./(Rk+Rh);
                                    Bi = drs(index1-1).*U./k;
                                    a = Rs(index1);
                                    ap = Rs(index1)+drs(index1+1)./2;
                                    A = ap./a;
                                    T_dt_new(index1) = T_dt_old(index1).*(1-2.*FoC.*(1+Bi.*A))+2.*FoC.*(T_dt_old(index1-1)+Bi.*A.*T_outer_air);
                                    dQ_outer_air1 = 2.*pi.*h.*Rs(index1).*(T_outer_air - T_dt_new(index1));

                                    
                                    % Outer Duct Outer surface Boudary:
                                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                                    FoC = FoX(Ts(index1+4),material_tag(index1+5),material_quality(index1+5),drs(index1+5),dt,FoC0);
                                    
                                    if mdot<=0
                                        air_dens = Pcold.*100000./278.05./T_outer_air;
                                        volume = pi.*(Rs(index1+4).^2-Rs(index1).^2);
                                        mass = air_dens.*volume;
                                        h = natural_convection_coefficient(Ts(index1+4),Ts(index1+2),Rs(index1+4),0,Pcold);
                                    else
                                        h = convection_coefficient(Ts(index1+2),Rs(index1+4),Rs(index1),Rs(index1+4),mdot,Pcold);
                                    end
                                    dT_on_2 = T_annular_radiation((T_dt_new(index1)-K0),Ts(index1+2),Ts(index1+4),Rs(index1),Rs(index1+4),Rs(index1+5),k_steel(Ts(index1+4)),h) - Ts(index1+4);
                                    
                                    k = insulation_k(material_tag(index1+4),material_quality(index1+4),(Ts(index1+4)+Ts(index1+5))./2);
%                                     if material_tag(index1+5)==6; k = k_outer_insulation((Ts(index1+5)+Ts(index1+4))./2); end
%                                     if material_tag(index1+5)==7; k = k_aluminium((Ts(index1+5)+Ts(index1+4))./2); end
                                    Rk = log((Rs(index1+4)+drs(+4))./Rs(index1+4))./(2.*pi.*k_steel(Ts(index1+4)));
                                    Rh = 1./(2.*Rs(index1+4).*pi.*h);
                                    U = 1./(Rk+Rh);
                                    Bi = drs(index1+5).*U./k;
                                    a = Rs(index1+4);
                                    am = Rs(index1+4)-drs(index1+5)./2;
                                    A = am./a;
                                    T_dt_new(index1+4) = T_dt_old(index1+4).*(1-2.*FoC.*(1+Bi.*A))+2.*FoC.*(T_dt_old(index1+5)+Bi.*A.*T_outer_air)+dT_on_2;
                                    dQ_outer_air2 = 2.*pi.*h.*Rs(index1+4).*(T_dt_new(index1+4) - T_outer_air);

                                    if mdot<=0
                                        dT_outer_air = (dQ_outer_air1 + dQ_outer_air2)./(mass.*air_Cp(T_outer_air-K0).*1000);
                                    else
                                        dT_outer_air = (dQ_outer_air1 + dQ_outer_air2)./(mdot.*air_Cp(T_outer_air-K0).*1000);
                                    end
                                    
                                    T_outer_air_new = T_outer_air + dT_outer_air;
                                    T_outer_air_new(T_outer_air_new<T_ambient) = T_ambient;
%                                     T_outer_air_new(T_outer_air_new>Tsteady.cold+K0) = Tsteady.cold+K0;
                                    T_dt_new(index1+2) = T_outer_air_new;
                                    T_dt_old(index1) = T_dt_new(index1);
                                    T_dt_old(index1+4) = T_dt_new(index1+4);
                                
                                    
                                    % Outer Surface - To Environement:
                                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                                    FoC = FoX(Ts(index2),material_tag(index2-1),material_quality(index2-1),drs(index2-1),dt,FoC0);
                                    
                                    h = natural_convection_coefficient(Ts(index2),T_ambient-K0,Rs(index2),wind_velocity,P_ambient);
                                    h3 = h;
%                                     dQ_to_amb = -2.*pi.*Rs(i).*h.*(T_dt_old(i)-T_ambient);
                                    k = insulation_k(material_tag(index2-1),material_quality(index2-1),(Ts(index2)+Ts(index2-1))./2);
%                                     if material_tag(index2-1)==6; k = k_outer_insulation((Ts(index2-1)+Ts(index2))./2); end
%                                     if material_tag(index2-1)==5; k = k_steel((Ts(index2-1)+Ts(index2))./2); end
                                    Rk = log((Rs(index2)+drs(index2))./Rs(index2))./(2.*pi.*k_aluminium(Ts(index2)));
                                    Rh = 1./(2.*Rs(index2).*pi.*h);
                                    U = 1./(Rk+Rh);
                                    Bi = drs(index2-1).*U./k;
                                    a = Rs(index2);
                                    ap = Rs(index2)+drs(index2-1)./2;
                                    A = ap./a;
                                    T_dt_new(index2) = T_dt_old(index2).*(1-2.*FoC.*(1+Bi.*A))+2.*FoC.*(T_dt_old(index2-1)+Bi.*A.*T_ambient);
                                    if T_dt_new(index2)<T_ambient; T_dt_new(index2)=T_ambient; end
                                    T_dt_old(index2) = T_dt_new(index2);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                                ...
                                for i = 3:length(Ts)-2
                                    ...
                                    if (i>3 && i<index1) || (i>(index1+4) && i<index2)
                                        % heat conduction - solids:
                                        if Rs(i-1)==Rs(i) %|| material_tag(i)==0
                                            T_dt_new(i) = T_dt_old(i-1);
                                        else
    ...
    B=[1,3:index1,index1+2,index1+4:index2];
...
% HTS Parameters:
        if HTS_type>1
            ...
            hLo = INPUT.HTS_hot.insulation_occupation;
            hLm = INPUT.HTS_hot.insulation_material;
            hLi = INPUT.HTS_hot.insulation_quality;

            cLt = INPUT.HTS_cold.insulation_thickness;
            cLm = INPUT.HTS_cold.insulation_material;
            cLi = INPUT.HTS_cold.insulation_quality;
        else
            R1 = INPUT.HTS_hot.duct_R;
            R2 = INPUT.HTS_cold.duct_R;
...
%% parameters:
    ...
    %% calculation:
                    ...
                    dT4K1 = (Tin1.*r_io+Tin2.*r_is2)./(2.*(-r_io+r_is2))-K0;
                    dT4K2 = (Tin2.*r_oo+Tin3.*r_os)./(2.*(-r_oo+r_os))-K0;
                    ...
                    Roc = log(r_oc./r_os)./(k_aluminium(dT4K2).*2.*pi); % (outer liner) 
                    ...
                    %components losses:
                    dTgas1_by_components = (8.5.*0.176.*(Tin2-Tr_ic)./Ric)./(mdot.*Cp1)./1000;
                    dTgas2_by_components = (3.1.*0.147.*(Tin3-Tr_oo)./Rambient)./(mdot.*Cp2)./1000;
